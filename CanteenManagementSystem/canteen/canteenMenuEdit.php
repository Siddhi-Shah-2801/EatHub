...
<head>
    ...
    if (isset($_POST["updateConfirm"])) {
        ...
        if ($updateResult) {
            header("location: canteenMenuDetail.php?foodId={$foodId}&up_fdt=1");
        } else {
            header("location: canteenMenuDetail.php?foodId={$foodId}&up_fdt=0");
        }
        exit(1);
    }
    include('../head.php');
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        ?>
        <form method="POST" action="canteenMenuEdit.php" class="form-floating" enctype="multipart/form-data">
            <h2 class="mt-4 mb-3 fw-normal text-bold"><i class="bi bi-pencil-square me-2"></i>Update Menu Detail</h2>
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="foodTodayAvailable" name="foodTodayAvailable" <?php if ($row["foodTodayAvailable"]) {
                    echo "checked";
                } ?>>
                <label class="form-check-label" for="foodTodayAvailable">Menu avaliable for today</label>
            </div>
            <div class="form-check form-switch mb-2">
                <input class="form-check-input" type="checkbox" id="foodPreOrderAvailable" name="foodPreOrderAvailable"
                    <?php if ($row["foodPreOrderAvailable"]) {
                        echo "checked";
                    } ?>>
            ...
            <div class="form-floating mb-2">
                <input type="number" step=".25" min="0.00" max="999.75" class="form-control" id="foodPrice"
                    placeholder="Price (Rs)" value="<?php echo $row["foodPrice"]; ?>" name="foodPrice" required>
